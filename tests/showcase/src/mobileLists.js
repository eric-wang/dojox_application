define(["dojo/_base/lang",
	...
	mobileLists = function(){
		...
		var _store1 = new ItemFileWriteStore({
			url: "data/dataList.json",
			clearOnClose: true
		});
		var _store2 = new ItemFileWriteStore({
			data: lang.clone(static_data)
		});
		return {
			store1: _store1,
			store2: _store2,
			...
			add1: function(){
				var item = this.listsStore.newItem({
					label: "New Item"
				});
			...
			init: function(){
				...
				var isInit = false;
				connect.connect(view, "resize", view, function(){
					if (!isInit) {
						this.resize();
						isInit = true;
					};
